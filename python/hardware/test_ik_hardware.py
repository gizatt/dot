...
def main():
    ...
    while not rospy.is_shutdown():
        t = time.time() - t0
        #leg_angles = do_ik(np.array([0.1, 0.1, -0.2]) + np.array([np.cos(t)*0.02, np.sin(t)*0.02, np.sin(t)*0.00]))
        # SQUATS
        hz = 0.5
        leg_angles = do_ik(np.array([0.0, 0.025, -0.25]) + np.array([0., 0., np.sin(t*hz*2.*np.pi)*0.05]))
